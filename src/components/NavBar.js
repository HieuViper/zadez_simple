                ...
                {data?.map((item, i) => (
                    <div key={i} className=''>
                        <div className="px-3 text-left group ">
                            {!item?.children ? <Link href={`/${item.category_code}`} style={{ textDecoration: "none", color: "black" }}>
                                <h1
                                    className="py-2 flex justify-between items-center md:pr-0 pr-5 text-base lg:text-xs 2xl:text-base md:cursor-pointer group-hover:text-red-500 text-black duration-300 transition"
                                >
                                    {item.name}
                                </h1>
                            </Link>
                            ...
                            {item?.children && (
                                <div className={`absolute ${scrollPosition > 0 ? 'top-8' : 'top-12'} hidden group-hover:md:block hover:md:block bg-white mt-5 py-2 pl-2 gap-10  rounded-xl shadow-lg  ease-in-out transition-all duration-500`}>
