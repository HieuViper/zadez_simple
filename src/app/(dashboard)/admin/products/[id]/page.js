...
const ProductForm = ({ params }) => {
  ...
  const uploadListImage = async () => {
    const newFiles = fileList && fileList.filter((file) => !initFileList?.some((initialFile) => initialFile?.name === file?.name));
    const initFiles = fileList && fileList.filter(file => initFileList?.some(initialFile => initialFile?.name === file?.name));
