  ...
  const columns = [
...

    {
      ...
      render: (_, record) => (
        <Space size="middle">
          <Link
            href={`/admin/articles/${record.id}?previousPage=${page}&previousLimit=${limit}`}
            prefetch={false}
          >
            Edit
          </Link>
          <Popconfirm
            title="Delete the articles"
            description="Are you sure to delete this articles?"
            onConfirm={async () => {
    ...
    <div>
      <div className="flex justify-between">
        <Button
          ...
          onClick={() =>
            router.push(
              `/admin/articles/0?previousPage=${page}&previousLimit=${limit}`
            )
          }
        >
          Add New
      ...
      <Table
        ...
        onChange={(e) => {
          router.replace(
            `/admin/articles?page=${e.current}&limit=${e.pageSize}&keyword=${search}`
          );
        }}
        rowKey="id"
      />
